...
const config = {
  timeout: env.FINGERPRINT_TIMEOUT,
  restart: false,
  timer: null,
};
    ...
    try {
      ...
      const { error, ...result } = await Promise.race(
        [
          ...
          params?.parameters?.perfectCanvasRequest
            ? null
            : new Promise((_, reject) => {
                setTimeout(
                  () =>
                    reject(
                      new Error(`Timed out while calling the "${name}" method.`)
                    ),
...
exports.fetch = (token, parameters, configuration) =>
  call("fetch", { token, parameters, ...configuration });

exports.setup = (proxy, fingerprint, configuration) =>
  call("setup", { proxy, fingerprint, ...configuration });
